...
class TwitterList(JSONTrait):
    ...
    defaultBannerMedia: str | None = None
    customBannerMedia: str | None = None
    _type: str = "snscrape.modules.twitter.TwitterList"

    @staticmethod
    def parse(obj: dict, res=None):
        # Parse creator user if available
        creator = None
        user_results = get_or(obj, "user_results.result", None)
        if user_results:
            # Try to build a minimal user from available data
            try:
                core = user_results.get("core", {})
                legacy = user_results.get("legacy", {})
                rest_id = user_results.get("rest_id", "")

                # Build legacy-compatible dict from core data
                user_data = {
                    "id_str": rest_id,
                    "screen_name": core.get("screen_name", legacy.get("screen_name", "")),
                    "name": core.get("name", legacy.get("name", "")),
                    "description": legacy.get("description", ""),
                    "created_at": core.get("created_at", legacy.get("created_at", "Thu Jan 01 00:00:00 +0000 1970")),
                    ...
                    "profile_image_url_https": get_or(user_results, "avatar.image_url", legacy.get("profile_image_url_https", "")),
                    "verified": legacy.get("verified", False),
                    "is_blue_verified": user_results.get("is_blue_verified", False),
                }
                if user_data["screen_name"]:
                    creator = User.parse(user_data)
            except Exception:
                pass

        # Parse created_at (can be timestamp or string)
        created_at = None
        ...
        if raw_created:
            try:
                if isinstance(raw_created, (int, float)):
                    ...
                    created_at = datetime.fromtimestamp(raw_created / 1000)
                else:
                    created_at = email.utils.parsedate_to_datetime(raw_created)
            except Exception:
                pass

        return TwitterList(
            id=int(obj.get("id_str", "0")),
            id_str=obj.get("id_str", ""),
            name=obj.get("name", ""),
            description=obj.get("description", ""),
            memberCount=obj.get("member_count", 0),
            subscriberCount=obj.get("subscriber_count", 0),
            mode=obj.get("mode", "Public"),
            isFollowing=obj.get("following", False),
            createdAt=created_at,
            creator=creator,
            defaultBannerMedia=get_or(obj, "default_banner_media.media_info.original_img_url"),
            customBannerMedia=get_or(obj, "custom_banner_media.media_info.original_img_url"),
        )
...
def parse_lists(rep: httpx.Response, limit: int = -1) -> Generator[TwitterList, None, None]:
    """Parse Twitter Lists from GraphQL response"""
    res = rep if isinstance(rep, dict) else rep.json()

    ids = set()

    def extract_lists(obj):
        """Recursively extract list objects from response"""
        if isinstance(obj, dict):
            # Check for TimelineTwitterList itemType (CombinedLists format)
            if obj.get("itemType") == "TimelineTwitterList" or obj.get("__typename") == "TimelineTwitterList":
                list_data = obj.get("list")
                if list_data:
                    yield list_data
            # Check if this is a direct list object
            elif obj.get("__typename") == "TwitterList" and obj.get("id_str"):
                yield obj
            # Recurse into nested structures
            for key, value in obj.items():
                if key not in ("list",):  # Don't recurse into already yielded list
                    yield from extract_lists(value)
        elif isinstance(obj, list):
            for item in obj:
                yield from extract_lists(item)

    for list_obj in extract_lists(res):
        if limit != -1 and len(ids) >= limit:
            break

        try:
            list_id = list_obj.get("id_str", "")
            if list_id and list_id not in ids:
                ids.add(list_id)
                yield TwitterList.parse(list_obj)
        except Exception as e:
