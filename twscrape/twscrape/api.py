...
from .models import Tweet, User, TwitterList, parse_trends, parse_tweet, parse_tweets, parse_user, parse_users, parse_lists
...
OP_CombinedLists = "ao5S9grv5By-qiuZKwaCpw/CombinedLists"
OP_ListByRestId = "9hIQYsj2RWaSc-LCWvZBcA/ListByRestId"
...
GQL_FEATURES = {  # search values here (view source) https://x.com/
    ...
    # New required features (2024-2025)
...
class API:
    ...
    # user_lists (combined lists - owned + subscribed)

    async def user_lists_raw(self, uid: int, limit=-1, kv: KV = None):
        op = OP_CombinedLists
        kv = {"userId": str(uid), "count": 100, **(kv or {})}
        ft = {
            "responsive_web_twitter_article_notes_tab_enabled": False,
        }
        async with aclosing(self._gql_items(op, kv, ft, limit=limit)) as gen:
    ...
    async def user_lists(self, uid: int, limit=-1, kv: KV = None):
        async with aclosing(self.user_lists_raw(uid, limit=limit, kv=kv)) as gen:
            async for rep in gen:
    ...
    async def list_by_id_raw(self, list_id: int, kv: KV = None):
        op = OP_ListByRestId
        kv = {"listId": str(list_id), **(kv or {})}
        ft = {
            "responsive_web_twitter_article_notes_tab_enabled": False,
        }
        return await self._gql_item(op, kv, ft)

    async def list_by_id(self, list_id: int, kv: KV = None) -> TwitterList | None:
        rep = await self.list_by_id_raw(list_id, kv=kv)
        if rep:
            lists = list(parse_lists(rep.json()))
